...
// who's there?
Hail { // now works locally 19/08/13
	...
	*new { |addrBook, period = 1.0, me, authenticator, oscPath = '/hail'|
	...
	makeOSCFuncs {
		...
		inOSCFunc = OSCFunc({|msg, time, addr|
			var name, senderPort, peer;
			var reconstitutedAddr;
			name = msg[1];
			senderPort = msg[2];
			reconstitutedAddr = NetAddr(addr.ip, senderPort);
			if(lastResponses[name].isNil, {
				peer = Peer(name, reconstitutedAddr);
		...
		outOSCFunc = OSCFunc({|msg, time, addr|
			var recvPort = msg[1];
			var reconstitutedAddr = NetAddr(addr.ip, recvPort);
			reconstitutedAddr.sendMsg(replyPath, me.name, addrBook.me.addr.port);
		}, oscPath).fix; // recvPort: addrBook.me.addr.port
	}
	...
	hailingSignal {
		...
		SystemClock.sched(0, {
			broadcastAddr.sendMsg(oscPath, addrBook.me.addr.port);
